...
V(:, end) = (min(S1_vec, S2_vec) <= K);  % S2 = 2*S2
V(end, :) = (min(S1_vec, S2_vec) <= K);  % S1 = 2*S1

% Compute coefficient matrices
a1 = dt * (sigma1 * S1_vec / dS1).^2;
a2 = dt * (sigma2 * S2_vec / dS2).^2;
b1 = dt * rho * sigma1 .* sigma2 .* S1_vec .* S2_vec / (dS1 * dS2);
b2 = b1;
c = 1 + r * dt;
...
for t = Nt:-1:1
    ...
    V_intermediate = A1 * V(2:N1, 2:N2);
    % Apply the digital-style payoff
    V_intermediate = max(V_intermediate, max(S1_vec(2:N1), S2_vec(2:N2)) <= K);
...
backward_dif_digital_optionValue = interp2(S2_vec, S1_vec, V, S2, S1)
